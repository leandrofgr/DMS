...
%     values below the optimal solution. In case you have no idea ofc
...
%             plot(h,gammaexp,'rs','MarkerSize',10);
            hold on
            fplot(@(h) funnugget(b) + func(b,h),[0 b(1)],'LineWidth',2)
            fplot(@(h) funnugget(b) + b(2),[b(1) max(h)],'LineWidth',2)
            
        case 'unbounded'
%             plot(h,gammaexp,'rs','MarkerSize',10);
            hold on
            fplot(@(h) funnugget(b) + func(b,h),[0 max(h)],params.color,'LineWidth',2);
    end
