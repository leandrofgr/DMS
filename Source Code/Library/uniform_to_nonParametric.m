...
for i = 1:1:size(data2transform,1) %para cada ponto repete;
    ...
    for j =1:1:size(data,2) %para cada variavel repete
        ...
        if size(invcumhist,1) > 1            
            ...
            disp('Not enough data for contitioning. The method will draw from the marginal for '+string(num_point_without_statistic)+' points. It might generate artifacts. Consider using KDE to increase the number of data points or increasing the grid_size parameter.')
