function S = cross_variogram(x,y,y2,varargin)             
...
% error checking
if (size(y,1) ~= size(x,1))||(size(y2,1) ~= size(x,1))
...
%lam      = (y(iid(:,1))-y2(iid(:,2))).^2;
lam      = (y(iid(:,1))-y(iid(:,2))).*(y2(iid(:,1))-y2(iid(:,2)));
...
if params.plotit
    ...
    if ~params.anisotropy
        plot(S.distance,S.val,marker,'color',params.color);
